...
import { ArrowRight, CheckCircle2, ClipboardCheck, FileText, Loader2, Paperclip, RefreshCw, ShieldCheck, X } from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { AgentTimeline } from '../chat/AgentTimeline';
import { formatDuration } from '../../services/registry';
import { WORKFLOWS, workflowPrompt, workflowCore, type WorkflowKind, type RunReceipt, type ReadinessReport, type NetworkEvent } from '../../services/workflows';
...
export const WorkflowView: React.FC = () => {
  ...
  const setDraft = (patch: Partial<typeof draft>) => setDrafts((all) => ({ ...all, [draftKey]: { ...(all[draftKey] ?? { notes: '', files: [] }), ...patch } }));
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try { const result = await workflowCore.receipts(session); if (!stopped) setReceipts({ session, rows: result }); }
  ...
  const launch = () => perform(async () => {
    if (!activeWorkspace || isRunning || draft.files.length < workflow.minimum) return;
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      {surface === 'workflows' && <>
        ...
        <div className={card}>
          ...
          <div className="mt-3 space-y-2">{draft.files.map((path, index) => <div key={path} className="flex gap-2 items-center text-xs"><span className="text-[var(--muted-foreground)]">{index + 1}.</span><span title={path} className="truncate flex-1">{path.split(/[\\/]/).pop()}</span><button aria-label={`Remove ${path.split(/[\\/]/).pop()}`} onClick={() => setDraft({ files: draft.files.filter((p) => p !== path) })}><X size={13} /></button></div>)}</div>
          <div className="mt-3 flex flex-wrap gap-2">
            <button className={button} disabled={busy || offline} onClick={() => void perform(async () => { const picked = await pickAttachments(); setDraft({ files: [...new Set([...draft.files,...picked])] }); })}><Paperclip size={13} />Attach sources</button>
            <button className={button} onClick={() => openTab('knowledge', 'Knowledge')}>Index SOPs</button>
            {kind === 'dashboard' && artifacts.some((a) => a.kind === 'xlsx' && a.sessionId === activeSessionId) && <button className={button} onClick={() => { const latest = artifacts.filter((a) => a.kind === 'xlsx' && a.sessionId === activeSessionId).sort((a,b) => b.createdAt-a.createdAt)[0]; if (latest) setDraft({ files: [...new Set([...draft.files,latest.path])] }); }}>Use latest tracker</button>}
          </div>
      ...
      {surface === 'receipts' && <>
        {!activeSessionId ? <p className="text-sm">Open a task to inspect its execution receipts.</p> : rows.length === 0 ? <p className="text-sm text-[var(--muted-foreground)]">No receipts recorded for this task yet. Earlier runs are not reconstructed as evidence.</p> : <>
      ...
      {surface === 'readiness' && <>
        ...
        {readiness && <><p className="text-xs text-[var(--muted-foreground)]">Checked {new Date(readiness.checkedAt).toLocaleTimeString()}. {readiness.scope}</p>{readiness.checks.map((c) => <div key={c.name} className={card}><p className="text-sm flex items-center gap-2">{c.status === 'pass' && <CheckCircle2 size={14} className="text-[var(--success)]" />}{c.name}<span className={`ml-auto text-xs ${c.status === 'pass' ? 'text-[var(--success)]' : 'text-[var(--warning)]'}`}>{c.status}</span></p><p className="mt-2 text-xs text-[var(--muted-foreground)] break-words">{c.detail}</p></div>)}</>}
      </>}
      {surface === 'network' && <>
